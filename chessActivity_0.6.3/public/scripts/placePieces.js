...
fetch("/echo/json/position", {
    ...
    fetchedPos = position
    console.log("Position fetched properly.")
    setupPositionFEN(fetchedPos)
})
...
async function placePiece(squareNum, pieceAlias) { //pieceAlias == position[index]
	...
	if (players[1] == currentPlayer) { // rotate sprite if black's POV
		piece.scale.y *= -1;
        piece.scale.x *= -1;
	}
...
function printGameInfo(position, posIndex) {
    ...
    if (posIndex < position.length) {
        ...
        if(position[posIndex] === "-"){
			...
			isEnpassantPossibleData = [true, ["abcdefgh".indexOf(position[posIndex]), turn == 0 ? 3 : 4]]
			posIndex += 3;
        }
        ...
        let temp = turn
        turn = (Number(fullmovesNumberString) - 1) * 2 + temp
        gameInfo.fullmoves += fullmovesNumberString

    }

    console.log(gameInfo)

    document.querySelector("body").style.backgroundColor = turn % 2 == 0 ? "rgb(179, 179, 179)" : "rgb(29, 29, 29)"
    reloadCheck()
