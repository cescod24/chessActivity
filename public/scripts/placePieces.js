...
let intervals = [] // this list contains all intervals created for pieces elimination
let spriteKiller = [] // this list contains booleans that point to the original position of a piece and determine if they must be eliminated or not
let rooksToCastle = []

for (let i = 0; i < 8; i++) {
    intervals.push([])
    for (let j = 0; j < 8; j++) {
        intervals[i].push(undefined)
        spriteKiller.push(false)
    }
}
...
setupPositionFEN(startingPosFEN)
...
async function setupPositionFEN(position) {
    ...
    while (boardIndex < 64) { // Sets up the position
        ...
        if (pieceAlias === " " || pieceAlias === "/") {
            ...
            currentPos[Math.floor(boardIndex / 8)].push(position[posIndex])
            currentToIndex[Math.floor(boardIndex / 8)].push(boardIndex)
            boardIndex++;
            posIndex++;
        }
...
async function placePiece(squareNum, pieceAlias) { //pieceAlias == position[index]
    ...
    if (pieceAlias === "R" || pieceAlias === "r") {
        rooksToCastle.push(piece)
    }
    // Add it to the stage
    app.stage.addChild(piece);


    intervals[Math.floor(squareNum / 8)][squareNum % 8] = setInterval(() => { // interval to eliminate a piece upon its capture

        if (spriteKiller[squareNum]) {
            app.stage.removeChild(piece)
            clearInterval(intervals[Math.floor(squareNum / 8)][squareNum % 8])
        }
...
function printGameInfo(position, posIndex) {
    ...
    if (posIndex < position.length) {
        position[posIndex] === "w" ? gameInfo.turn = "White's turn" : position[posIndex] === "b" ? gameInfo.turn = "Black's turn" : gameInfo.turn = "N/A"
        posIndex += 2;
        ...
        let halfmovesNumberStrign = ""
        gameInfo.halfmoves = "Halfmoves: "
        while(position[posIndex] !== " ") {
            gameInfo.halfmoves += position[posIndex]
            halfmovesNumberStrign += position[posIndex]
            posIndex++;
        }
        posIndex++;
        ...
        turn = Number(fullmovesNumberString - 1) * 2 + Number(halfmovesNumberStrign)
