...
function onDragEnd()
{
    ...
    if (isSquareAvailable(initialSquare, x_destination, y_destination) && basicMovement(initialSquare, x_destination, y_destination)) {
        dragTarget.x = x_destination
        dragTarget.y = y_destination
        logicManager(initialSquare, x_destination, y_destination)
        if (justCastled) moveRook(x_destination, y_destination)
    } else {
