function basicMovement(initialSquare, x, y) {
    ...
    switch (selectedPieceType) {
        case "P" :
            if (initialSquare[0] == targetSquare_x && targetSquare_y + 2 == initialSquare[1] && initialSquare[1] == 6 && currentPos[targetSquare_y][targetSquare_x] == "/") isPermitted = true // move by two
            ...
            if (isEnpassantPossible(targetSquare_x, targetSquare_y, initialSquare[0], initialSquare[1])) isPermitted = true
            break
        case "p" :
        ...
        case "R" :
            ...
            if (initialSquare[1] == targetSquare_y && initialSquare[0] != targetSquare_x && !rookHasJumpedOver(initialSquare[0], targetSquare_x, targetSquare_y, "horizontal")) isPermitted = true // rook diagonal
            break
        case "n" :
        case "N" : 
            if (Math.pow(initialSquare[0] - targetSquare_x, 2) + Math.pow(initialSquare[1] - targetSquare_y, 2) == 5) isPermitted = true // knight (so smart again w remonds)
            break
        case "b" :
        case "B" :
        ...
        case "K" :
            ...
            if (targetSquare_x == initialSquare[0] + 2 || targetSquare_x == initialSquare[0] - 2) isPermitted = canCastle(initialSquare[0], initialSquare[1], targetSquare_x, targetSquare_y, selectedPieceType) // can castle? 
            break
    }

    if (currentPos[targetSquare_y][targetSquare_x] === "k" || currentPos[targetSquare_y][targetSquare_x] === "K") isPermitted = false
